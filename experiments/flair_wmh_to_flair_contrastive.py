# -*- coding: utf-8 -*-
"""
Experiment 6: FLAIR + WMH → FLAIR with downstream segmentation (L1 + SSIM loss)
"""
...
class Experiment6(BaseExperiment):
    """
    Experiment 6: FLAIR + WMH → FLAIR prediction with downstream WMH segmentation.
    Uses L1 + SSIM loss for segmentation
    """
    
    def __init__(self, experiment_number, experiment_config, config):
        """
        Initialize Experiment 6.
    ...
    def run(self):
        """Execute the full Experiment 6 pipeline."""
        print("\n" + "="*60)
        print("Starting Experiment 6: FLAIR + WMH → FLAIR (L1 + SSIM loss)")
        print("="*60 + "\n")
...
if __name__ == "__main__":
    ...
    Usage: python experiments/flair_wmh_to_flair_contrastive.py
    """
    print("\n" + "="*70)
    print("🧪 Running Experiment 6: FLAIR + WMH → FLAIR (L1 + SSIM loss)")
    print("="*70 + "\n")
    ...
    experiment_config = {
        ...
        "description": "FLAIR + WMH-> FLAIR (two-stage: prediction then segmentation, loss: L1 + SSIM)",
        "use_wmh": True,
        "class": Experiment6
    }
