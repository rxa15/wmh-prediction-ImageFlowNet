# -*- coding: utf-8 -*-
"""
Experiment 4: FLAIR → FLAIR (dense pairs) prediction with downstream WMH segmentation
Using L1 + SSIM loss, separate forward passes per (source, target, Δt) pair
"""
...
class Experiment4(BaseExperiment):
    """
    Experiment 4: FLAIR → FLAIR (dense-pairs: ALL POSSIBLE PAIRS) prediction with downstream WMH segmentation
    Using L1 + SSIM loss, separate forward passes per (source, target, Δt) pair
    """
    ...
    def run(self):
        """Execute the full Experiment 4 pipeline."""
        print("\n" + "="*60)
        print("Starting Experiment 4: FLAIR → FLAIR (dense pairs) prediction with downstream WMH segmentation")
        print("="*60 + "\n")
    ...
    def _stage1_train_imageflownet(self):
        ...
        print("="*60)
        print("✅ STAGE 1: ImageFlowNet Training (dense pairs)")
        print("="*60)
        
        training_pairs = [
            ("Scan1Wave2", "Scan2Wave3", 1.0),  # t1 -> t2
            ("Scan1Wave2", "Scan3Wave4", 2.0),  # t1 -> t3
            ("Scan1Wave2", "Scan4Wave5", 3.0),  # t1 -> t4
            ("Scan2Wave3", "Scan3Wave4", 1.0),  # t2 -> t3
            ("Scan2Wave3", "Scan4Wave5", 2.0),  # t2 -> t4
            ("Scan3Wave4", "Scan4Wave5", 1.0),  # t3 -> t4
        ]
        
        print("Initializing dataset with custom training pairs ...")
        full_dataset = FLAIREvolutionDataset(
            root_dir=self.config["ROOT_DIR"],
            max_slices_per_patient=self.config["MAX_SLICES"],
            use_wmh=self.use_wmh,
            training_pairs=training_pairs  # Train on multiple horizons
        )
...
if __name__ == "__main__":
    ...
    Usage: python experiments/flair_to_flair_dense_pairs_L1_SSIM.py
    """
    print("\n" + "="*70)
    print("🧪 Running Experiment 4: FLAIR → FLAIR (dense pairs, L1 + SSIM)")
    print("="*70 + "\n")
    ...
    experiment_config = {
        ...
        "description": "FLAIR -> FLAIR (dense pairs) prediction with downstream WMH segmentation",
        "use_wmh": True,
        "class": Experiment4
    }
