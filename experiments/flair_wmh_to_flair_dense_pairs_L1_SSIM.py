# -*- coding: utf-8 -*-
"""
Experiment 8: FLAIR + WMH → FLAIR (dense-pairs) prediction with downstream WMH segmentation.
Using L1 + SSIM loss, separate forward passes per (source, target, time_delta) pair.
"""

from base import BaseExperiment
from utils_laras_multimodal import (
...
class Experiment8(BaseExperiment):
    ...
    def run(self):
        """Execute the full Experiment 8 pipeline."""
        print("\n" + "="*60)
        print("Starting Experiment 8: FLAIR + WMH → FLAIR (dense-pairs, L1 + SSIM loss)")
        print("="*60 + "\n")
    ...
    def _stage1_train_imageflownet(self):
        ...
        print("Initializing dataset with custom training pairs (dense pairs)...")
    ...
    def _train_fold(self, val_fold_idx, full_dataset, folds_dict):
        ...
        # Initialize model: FLAIR (ch0) + WMH (ch1) -> FLAIR (1 ch)
        model = ImageFlowNetODE_FlexibleOutput(
            device=self.config["DEVICE"],
            in_channels=2, # FLAIR + WMH
            out_channels=1, # FLAIR only
...
if __name__ == "__main__":
    ...
    Usage: python experiments/flair_wmh_to_flair_dense_pairs_L1_SSIM.py
    """
    print("\n" + "="*70)
    print("🧪 Running Experiment 8: FLAIR + WMH → FLAIR (dense pairs, L1 + SSIM)")
    print("="*70 + "\n")
    ...
    experiment_config = {
        ...
        "description": "FLAIR + WMH-> FLAIR (dense pairs) prediction with L1 + SSIM loss",
        "use_wmh": True,
        "class": Experiment8
    }
